...
df_AUC_PFU <- tibble(group = character (),
                      host=character(), 
                      unit = character(),
                      AUC_avg_titre = numeric())

# Subset data to only include rows where unit is 'PFU'
data_filtered <- subset(data_final, unit == "PFU")
...
for (h in 1: length(by_host)){
  ...
  jpeg(paste(df$host_figure_name[1],'_PFU.jpeg', sep=''), units='in', width = 5, height = 4, res=1200, pointsize=6) 
  par(mar = c(5,5,5,2))
  plot(NA, NA, ylim=c(0,max(data_final$max_peak_titre))*1.05, xlim=c(0,8), type='o', pch=16, 
       main=ifelse(df$unit[1]=='PFU', paste(df$host_figure_name[1], '- PFU'), df$host_figure_name),
  ...
  for (i in 1:nrow(df)){
    ...
    # calculate AUC
    if (df$unit[i]=='PFU') {# only calculates AUC for PFU-based curves
      AUC_avg <- trapz(x_avg_plot, pred_avg_plot)
      df_AUC_PFU <- df_AUC_PFU %>% add_row(group = df$group[i],
...
df_AUC_scaled_PFU <- left_join(df_AUC_PFU, data_final, by = c('group', 'unit')) # join AUC and p_viraemic data
df_AUC_scaled_PFU<- df_AUC_scaled_PFU%>%
  mutate(df_AUC_scaled_PFU, weighted_AUC = AUC_avg_titre*prop_viraemic) %>% # calculate weighted AUC column
  mutate(df_AUC_scaled_PFU, figure_name = paste(figure_name, sep = ' - ')) %>%
...
pal<-colfunc(1000)
cols_avg <- pal[with(df_AUC_scaled_PFU, round(weighted_AUC/max(weighted_AUC)*1000,0))]
jpeg('PFU.jpeg', units='in', width = 5, height = 2.4, res=1200, pointsize=6) 
par(mar=c(5,20,3,15))
plot(NA, NA, ylim=c(0, nrow(df_AUC_scaled_PFU)+1), xlim=c(0, 15), 
     yaxs = 'i', main='',
     xlab = expression('(AUC of viraemia curve)*(proportion viraemic)'),
     yaxt = 'n', ylab='', cex.main = 1.75, cex.lab = 1.2, cex.axis =1.25)
index <- nrow(df_AUC_scaled_PFU)
for (i in 1:index){
  lines(x = c(-0.4,df_AUC_scaled_PFU$weighted_AUC[i]),
        y = c(index+(1-i), index+(1-i)), col=alpha(cols_avg[i], 0.5), lwd=3)}
with(df_AUC_scaled_PFU,{
  point_colors <- ifelse(weighted_AUC == 0, "grey", cols_avg)
  points(x = weighted_AUC, y=index:1, type='p', pch=16, cex = 2, col = point_colors)
})

mtext(side=2, text=df_AUC_scaled_PFU$scientific, at=index:1, line=8, las=1, cex=0.8, adj=0)
mtext(side=2, text=df_AUC_scaled_PFU$figure_name, at=index:1, line=19, las=1, cex=1.2, adj=0)
mtext(side=4, text=df_AUC_scaled_PFU$sample_size, at=index:1, line=3.1, las=1, cex=1.2, adj=0.5)  
mtext(side=4, text=df_AUC_scaled_PFU$prop_viraemic, at=index:1, line=8.5, las=1, cex=1.2, adj=0.5)  
mtext(side=4, text = 'PFU', line=-3.0, padj=-4.8, las=1, cex=1.7, font=2)
